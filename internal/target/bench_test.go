...
// Shared benchmark directories created once in TestMain
var (
...
func TestMain(m *testing.M) {
	// Create temp root for all benchmark trees
	...
	go func() {
		defer wg.Done()
		benchDirSmall = createBenchmarkTreeOnce(filepath.Join(tempRoot, "small"), 3, 100)
	}()
	go func() {
		defer wg.Done()
		benchDirMedium = createBenchmarkTreeOnce(filepath.Join(tempRoot, "medium"), 5, 100)
	}()
	go func() {
		defer wg.Done()
		benchDirLarge = createBenchmarkTreeOnce(filepath.Join(tempRoot, "large"), 8, 10)
	}()
